...
a mean end-to-end latency of 8.2\,s (SD\,$=$\,1.9\,s) and a Socratic
...
platforms~\cite{roddiger2022,kawsar2018}, yet their application to
educational scaffolding has not been explored.
...
We use Llama~3.2~1B~\cite{touvron2023} via Ollama (temperature~0.2,
60-token limit), synthesised with \texttt{en-US-AriaNeural} via
\texttt{edge-tts}. \texttt{Cmd+Shift+H} replays the previous hint for
a deeper follow-up; \texttt{Escape} dismisses it.
...
\noindent\textit{This section characterises system performance; a
controlled user study with novice programmers is planned as future work.}
...
\caption{Latency breakdown across 20 simulated stuck events.}
\label{tab:latency}
\begin{tabular}{lcccc}
\toprule
...
LLM inference  & 6.1 & 1.5 & 4.1 & 10.9 \\
TTS synthesis  & 1.4 & 0.4 & 0.9 & 2.3  \\
\midrule
\textbf{Total} & \textbf{8.2} & \textbf{1.9} & \textbf{5.8} & \textbf{13.4} \\
\bottomrule
\end{tabular}
\end{table}

The 8.2\,s mean arrives while the student's mental model remains active
...
\textbf{Hint quality.} Two raters scored all 20 hints on a 3-point
Socratic rubric (inter-rater $\kappa\!=\!0.71$). When runtime error was
captured, mean rating was \textbf{2.6/3.0}; without a crash (silent
logical error), it dropped to 1.4/3.0, confirming runtime error capture
is the primary quality driver.

\textbf{Stuck detection precision.} Ten one-hour developer sessions were
replayed offline; the scorer fired 43 times, of which 37 coincided with
visually observable struggle (fixed gaze, repeated failed edits),
...
exhaustive annotation of stuck boundaries and is a target for the
planned user study.
...
\textbf{Latency.} At 8.2\,s mean, hints arrive within Sweller's
...
largely unexplored.
All data, code, and session logs are available at an anonymised
repository link.
